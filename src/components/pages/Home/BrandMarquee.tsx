...
import Image from "next/image";
import Marquee from "react-fast-marquee"; // Default import, not named import
...
export default function BrandMarquee() {
  return (
    <div className="relative z-10 w-full overflow-hidden rounded-2xl bg-site-panel py-2 shadow-[var(--site-shadow)]">
      <Marquee speed={40} gradient={false} pauseOnHover>
        {brandLogos.map((brand, index) => (
          <div
            key={brand.alt}
            className="mx-6 flex h-14 min-w-[120px] items-center justify-center opacity-80 transition-all duration-300 hover:scale-105 hover:opacity-100 sm:mx-8 sm:h-12 sm:min-w-[140px] md:mx-10 md:h-16 md:min-w-[160px]"
          >
            <Image
              src={brand.src}
              alt={brand.alt}
              width={160}
              height={40}
              className="max-h-8 w-auto object-contain brightness-0 transition-all duration-300 group-hover:opacity-100 sm:max-h-10 md:max-h-12"
              priority={index < 3}
